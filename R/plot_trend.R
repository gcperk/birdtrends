...
#' @examples
#' \dontrun{
#'   hgams_plot <- trend_plot(raw_indices = input_option_1, model_indices = ldf_hgam, pred_indices = preds_hgam, start_yr = 2014, end_yr = 2022)
#'}
trend_plot <- function(raw_indices = indat1,
  ...
  sp_plot_index <- ggplot() +
    ...
    # Observed indices
    geom_errorbar(data = subset(raw_indices, year <= max(raw_indices$year)),aes(x = year, ymin = index_q_0.025, ymax = index_q_0.975), width = 0, col = "gray30")+
    geom_point(data = subset(raw_indices, year <= max(raw_indices$year)),aes(x = year, y = index), col = "gray30")+
